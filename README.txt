Мухаметгалиева Замира
код приложения «Movie Library» на Python с использованием Tkinter. Программа реализует все указанные требования: добавление фильмов, фильтрацию, сохранение и загрузку данных из JSON, а также проверку корректности ввода.

Инструкция по запуску
Создайте файл с именем movie_library.py.
Скопируйте и вставьте в него код ниже.
Запустите файл командой в терминале:
python movie_library.py
Если у вас не установлен Tkinter (в некоторых дистрибутивах Linux), установите его: sudo apt-get install python3-tk.
...
if __name__ == "__main__":
    ...
    root.mainloop()
Как это работает
Интерфейс: Программа открывается в новом окне. Слева расположены поля для ввода данных о фильме и кнопки.
Добавление: Введите данные и нажмите «Добавить фильм». Если данные некорректны (например, текст в поле «Год» или рейтинг 15), появится сообщение об ошибке.
Фильтрация: Введите жанр (например, comedy) или год (например, 2023) и нажмите «Применить фильтр». Таблица обновится.
Сохранение: Все добавленные фильмы сохраняются в файл movies.json в той же папке. При следующем запуске они автоматически загрузятся в таблицу.